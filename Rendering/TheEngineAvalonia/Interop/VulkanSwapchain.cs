    ...
    public void BeginDraw()
    {
        ...
        else if (_timelineSemaphore != null)
        {
            unsafe
            {
                ...
                // a non-empty wait MUST carry a matching dst stage mask (VUID-VkSubmitInfo-pWaitDstStageMask);
                // the original GpuInterop demo omitted it, which is UB and faults under load on MoltenVK.
                buffer.Submit(waitSemaphores, new[] { PipelineStageFlags.AllCommandsBit }, pNext: (IntPtr)(&submitInfo));
            }
        }
        else if (_initial)
        {
