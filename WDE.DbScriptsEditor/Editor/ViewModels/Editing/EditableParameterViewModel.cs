...
{
    // Copy-adapted from WDE.EventAiEditor (minus SpecialCommand support, which dbscripts don't
    // use): one editable parameter row of the edit dialog, rendered as a completion combo box
    // (items), flags combo (flag parameters), checkbox (bools) or a plain value box with picker.
